...
export class Database {
    constructor(connectionString = process.env.DATABASE_URL || "") {
        if (!connectionString) {
            throw new Error("DATABASE_URL must be set or passed as connectionString");
        }
        this.pool = new Pool({
            connectionString,
            max: 20,
    ...
    async runMigrations() {
        ...
        try {
            // Read migration file
            const migrationPath = path.join(__dirname, "../../migrations/001_readiness_schema.sql");
     ...
     */
    async getAllBlockers(tenant_id) {
        const result = await this.pool.query(`SELECT
        id, tenant_id, blocker_type, blocker_category, severity,
        description, blocking_record_ids, created_at, resolved_at, reopened_at
       FROM blockers
    ...
    async createBlocker(blocker) {
        const result = await this.pool.query(`INSERT INTO blockers
        (id, tenant_id, blocker_type, blocker_category, severity,
         description, blocking_record_ids, created_at, resolved_at, reopened_at)
     ...
     */
    async resolveBlocker(blocker_id, resolved_at) {
        const result = await this.pool.query(`UPDATE blockers
       SET resolved_at = $2
       WHERE id = $1
       RETURNING *`, [blocker_id, resolved_at]);
     ...
     */
    async getSourceFreshness(tenant_id) {
        const result = await this.pool.query(`SELECT
     ...
     */
    async getSource(tenant_id, source_name) {
        const result = await this.pool.query(`SELECT
        id, tenant_id, source_name, source_type, last_success_at,
        last_failure_at, state, staleness_threshold_seconds, updated_at
       FROM source_freshness
       WHERE tenant_id = $1 AND source_name = $2`, [tenant_id, source_name]);
        if (result.rows.length === 0) {
            return null;
        }
     ...
     */
    async upsertSourceFreshness(record) {
        const result = await this.pool.query(`INSERT INTO source_freshness
        (id, tenant_id, source_name, source_type, last_success_at,
         last_failure_at, state, staleness_threshold_seconds, updated_at)
